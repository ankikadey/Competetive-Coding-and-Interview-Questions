...
private:
    bool bfs(const string &beginWord, const string &endWord, unordered_set<string> &wordSet, unordered_map<string, int> &distFromBeginWord)
    {
    ...
    void dfs(const string &word, const string &beginWord, const unordered_map<string, int> &distFromBeginWord, const unordered_set<string> &wordSet, vector<string> &&path, vector<vector<string>> &ans)
    {
        if (word == beginWord)
        {
        ...
        {
            if (const auto it = distFromBeginWord.find(child); it != distFromBeginWord.cend() && it->second == currDist - 1)
            {
    ...
    vector<string> getChildren(const string &parent, const unordered_set<string> &wordSet)
    {
